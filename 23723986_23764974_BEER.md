...
The handle_clients function effectively addresses the requirement of disconnection handling in a multiplayer server context. By wrapping the entire game logic in a try-except block, the function detects common disconnection errors such as ConnectionResetError, BrokenPipeError, and other OSError instances that typically arise when a socket is unexpectedly closed. Upon detecting such an event, it handles the disconnection gracefully by informing the remaining player that the opponent has disconnected, treating it as an immediate forfeit. This prevents the server from crashing or hanging. The use of a finally block ensures that all resources—including socket connections and file objects—are safely closed, even in the event of an error. Moreover, the game_lock mechanism ensures that only one game session runs at a time, maintaining server stability. After each session, whether completed normally or terminated due to disconnection, the server remains operational and checks if another match can be launched using launch_game_if_ready(). 
...
The provided lobby_manager and launch_game_if_ready functions meet the requirement of handling idle or extra clients by implementing a waiting lobby system. When a new client connects, they are added to a shared lobby list under a lobby_lock to ensure thread-safe access. If there are at least two players in the lobby and no game is currently running (ensured by checking if game_lock is not locked), the server starts a new game by removing the first two clients from the lobby and assigning them to a new thread to handle the game session via handle_clients. This approach prevents rejecting additional connections and provides a user-friendly experience by keeping extra clients engaged until a game becomes available. The message [INFO] You are in the lobby informs clients of their waiting status, enhancing transparency. 
...
In our implementation there were several security vulnerabilities that we identified including the lack of session authentication, absence of protection against replay attacks, and no cryptographic binding between messages and users. For example, the server accepted any message with a valid checksum, even if it had already been received. This allowed an attacker to capture a valid encrypted message like "A5" with its checksum and replay it later to disrupt the game, as there was no validation of the message’s time or session association. To mitigate this, we could modify the communication protocol by adding a timestamp and session ID, not just the message and the checksum. This structure would be in the form; timestamp, session id, message, checksum which would ensure that the session ID is valid and the timestamp is within an acceptable time window to prevent replay attacks. 
