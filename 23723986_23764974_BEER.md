...
Task 2-1:   
To meet the Tier 2.1 requirement, we added several layers of input validation to ensure server stability and user feedback for invalid actions:
- Turn Enforcement: Input is only read from the player whose turn it is. Other clients cannot interfere out-of-turn due to the sequential structure of the server’s main loop.
- Empty Input Handling: If a player sends an empty message (e.g., presses Enter), the server responds with a prompt to enter a valid coordinate.
- Invalid Format Handling: Malformed coordinates or non-grid inputs are gracefully handled using try/except blocks around parse_coordinate(), which raises a ValueError. This prevents crashes and guides the player to correct their input.
- Repeated Shots: If a player tries to fire at the same coordinate more than once, the server detects this (result == 'already_shot') and asks the player to retry.
This combination of proactive validation and defensive programming ensures the server never crashes from bad input and improves the overall user experience.
