...
Each packet sent includes a header prepended to the message which contains either a sequence number (for client -> server communication) or acknowledgement number (for server -> client communication). The client assigns incrementing sequence numbers in the `timed_input()` function to each outgoing message. On the server side, once a message is received the server verifies that the sequence number is strictly greater than the previous sequence number received. If this condition is satisifed, the server responds with a message containing the last valid sequence number in the header, this represents an acknowledgement. Messages sent with a header are formatted as `header|message`

In cases where a server is sending a message to a spectator, the message does not contain and aknowledgement in the header as messages sent by spectators do not affect gameplay and therefore do not generate sequence numbers. Here, the message is formatted as `|message` to represent an empty header. 
