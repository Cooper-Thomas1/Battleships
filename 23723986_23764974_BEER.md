...
Task 2-1:   
The run_two_player_game_online function in battleship.py ensures invalid commands are delt with appropriately. Specifically, the server makes sure players alternate turns by using current = 1 - current, which allows Player 1 to fire at Player 2's board and vice versa. The input validation is handled by the parse_coordinate function, which ensures valid coordinates; if a player sends invalid input like "Z99", the server responds with "Invalid input: Invalid coordinate format." Similarly, if a player fires at the same location twice, the server replies with "You've already fired at that location. Try again." The game ends gracefully when all ships on a player's board are sunk, with messages like "Congratulations! You sank all ships in 15 moves." sent to the winner and "All your ships are sunk. You lose." sent to the loser. Additionally, if a player sends "quit", the game terminates, notifying the opponent with "Opponent forfeited. You win!"
